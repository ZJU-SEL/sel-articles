...
本文描述karmda的上手流程，使用的karmada版本为v0.7.0，发布于2021年7月12日，具体commit：c4835e1f。
...
`karmada-host` context对应的控制面运行在`kube-system` namespace中，用来运行管理karmada控制面，是个由kind启动的标准的k8s管理节点。TODO：这个控制面是不是有点多余？直接运行karmada控制面不行吗？
...
1. 重用了k8s的两个组件：`kube-apiserver`和`kube-controllermanager`
	> 虽然重用了组件，但karmada控制面中的kube-controller-mamager通过启动flag限制其只运行namespace、garbagecollector、serviceaccount-token、serviceaccount这几个controller，所以当把`Deployment`等k8s标准资源提交给karmada apiserver时，只是被记录在karmada控制面的etcd中，并在后续同步到member集群中，并不会发生`Deployment`资源在karmada-apiserver集群中的reconcile（如创建pod）。
...
1. 执行`karmadactl join member1 --cluster-kubeconfig=$HOME/.kube/karmada.config`以push的方式把member1加入karmada集群  
	> 如果没有karmadactl可执行文件，可以现在代码根目录执行make karmadactl
...
karmada代码里自带一个nginx应用可以作为用来上手分布式工作负载管理：
	  ...
	  placement:
        clusterAffinity:
          clusterNames:
            - member1
	```
	明确将之前部署的nginx deployment资源同步到member1集群中
1. 这时不用切换到member1 context，对karmada控制面执行`kubectl get deploy`就可以得到如下输出：
	```sh
	NAME    READY   UP-TO-DATE   AVAILABLE   AGE
	nginx   1/1     1            1           21m
	```
	说明karmada有能力从member集群同步工作负载状态到host集群。作为验证，可以切换到member1集群，执行`kubectl get po`可以看到nginx已经正常运行：
	```sh
